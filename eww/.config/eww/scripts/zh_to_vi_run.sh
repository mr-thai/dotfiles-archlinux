...
eww update detail_source="$DISPLAY_SRC" detail_result="⏳ Translating..." detail_status="󰚩 ZH → VI"
eww open detail_popup 2>/dev/null || true

if python -c "import pypinyin" &> /dev/null; then
    PINYIN=$(python -c "import sys; from pypinyin import pinyin; print(' '.join([x[0] for x in pinyin(sys.argv[1])]))" "$SRC_TEXT" 2>/dev/null)
    if [ -n "$PINYIN" ]; then
        DISPLAY_SRC="$SRC_TEXT ($PINYIN)"
...
[ -n "$DST_TEXT" ] && printf '%s\t%s\t%s\t%s\n' \
    "$(date '+%Y-%m-%d %H:%M')" "ZH→VI" "$DISPLAY_SRC" "$DST_TEXT" >> "$HIST_FILE"
