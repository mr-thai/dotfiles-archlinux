...
eww update detail_source="$SRC_TEXT" detail_result="⏳ Translating..." detail_status="󰚩 VI → ZH"
eww open detail_popup 2>/dev/null || true

DEEPL_KEY="2999ff54-b53b-42fd-b2a4-25f0a202ab7d:fx"
JSON_PAYLOAD=$(jq -n --arg text "$SRC_TEXT" --arg target "ZH" '{text: [$text], target_lang: $target}')
...
if [ -n "$DST_TEXT" ] && [ "$DST_TEXT" != "⏳ Timeout." ] && [[ "$DST_TEXT" != *"❌"* ]]; then
    if python -c "import pypinyin" &> /dev/null; then
        PINYIN=$(python -c "import sys; from pypinyin import pinyin; print(' '.join([x[0] for x in pinyin(sys.argv[1])]))" "$DST_TEXT" 2>/dev/null)
        DST_TEXT="$DST_TEXT ($PINYIN)"
    fi
fi
...
[ -n "$DST_TEXT" ] && printf '%s\t%s\t%s\t%s\n' \
    "$(date '+%Y-%m-%d %H:%M')" "VI→ZH" "$SRC_TEXT" "$DST_TEXT" >> "$HIST_FILE"
